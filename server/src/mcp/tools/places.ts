...
export function registerPlaceTools(server: McpServer, userId: number, scopes: string[] | null): void {
  ...
  if (W) server.registerTool(
    ...
    async ({ tripId, placeId, name, description, lat, lng, address, category_id, price, currency, duration_minutes, notes, website, phone, transport_mode, osm_id, google_place_id }) => {
      if (isDemoUser(userId)) return demoDenied();
      if (!canAccessTrip(tripId, userId)) return noAccess();
      if (!hasTripPermission('place_edit', tripId, userId)) return permissionDenied();
      const place = updatePlace(String(tripId), String(placeId), { name, description, lat, lng, address, category_id, price, currency, duration_minutes, notes, website, phone, transport_mode, osm_id, google_place_id });
